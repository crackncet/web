...
import { useMemberTestSeriesQuery } from "./_queries/test-series.queries";
import {
  Search,
  BookOpen,
  RefreshCw,
...
import { MemberTestSeriesCard } from "./_components/member-test-series-card";

export default function MemberTestSeriesPage() {
  ...
  // Queries
  const { data: responseData, isLoading: isTestSeriesLoading, isPlaceholderData } = useMemberTestSeriesQuery({
    page,
    limit,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* Test Series Grid / Skeletons / Empty State */}
      {isTestSeriesLoading ? (
        ...
        <Card className="border border-dashed border-border/80 bg-muted/5 py-12 flex flex-col items-center justify-center text-center p-6 select-none rounded-xl">
          ...
          <h3 className="font-bold text-sm text-foreground">No Test Series Found</h3>
          <p className="text-xs text-muted-foreground max-w-sm mt-1 leading-relaxed">
            You are not assigned to compile questions for any CBT mock exam series matching the filters.
          </p>
          <Button
            variant="outline"
            size="sm"
          ...
          {meta.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-900 rounded-xl select-none shadow-xs">
              <span className="text-xs sm:text-sm text-muted-foreground font-normal">
                ...
                <span className="font-semibold text-foreground">{meta.total}</span> test series
              </span>
