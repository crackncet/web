...
interface MemberTestSeriesCardProps {
  testSeries: TestSeries;
}

export function MemberTestSeriesCard({ testSeries }: MemberTestSeriesCardProps) {
  ...
  return (
    <Card className={`group overflow-hidden border-0 bg-white dark:bg-slate-900 flex flex-col h-full select-none ${shadowClass}`}>
      ...
      <div className="w-full pt-4 px-4 flex justify-center">
        <div className="relative aspect-video w-[90%] overflow-hidden rounded-xl bg-slate-100 dark:bg-slate-855 border border-slate-200/40 dark:border-slate-800 shadow-xs">
          {testSeries.banner ? (
            <Image
      ...
      <CardContent className="relative bg-white dark:bg-slate-900 pt-5 px-5 pb-5 flex-1 flex flex-col justify-between -mt-4 z-20">
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 space-y-3.5">
          ...
          <div className="flex items-center pt-0.5">
            <Link href={`/dashboard/member/test-series/${testSeries.id}`} className="w-full">
              <Button 
                variant="outline" 
