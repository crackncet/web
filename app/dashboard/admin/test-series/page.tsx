...
import { AdminTestSeriesCard } from "./_components/admin-test-series-card";
import { CreateTestSeriesDialog } from "./_components/create-test-series-dialog";
      ...
      {isTestSeriesLoading ? (
        ...
        <Card className="border border-dashed border-border/80 bg-muted/5 py-12 flex flex-col items-center justify-center text-center p-6 select-none rounded-xl">
          ...
          <p className="text-xs text-muted-foreground max-w-sm mt-1 leading-relaxed">
            There are no test series matching your query or selected filters. Try adjusting or resetting the active filters.
          </p>
          <Button
            variant="outline"
            size="sm"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {testSeriesList.map((ts) => (
              <AdminTestSeriesCard
                key={ts.id}
                testSeries={ts}
                examName={getExamName(ts.examId)}
              />
            ))}
          </div>
