...
interface FinancesPaginationProps {
  page: number;
  limit: number;
  totalPages: number;
  total: number;
...
export function FinancesPagination({
  page,
  limit,
  totalPages,
  total,
  onPageChange,
}: FinancesPaginationProps) {
    ...
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 border-t border-border bg-card">
      ...
      <div className="flex items-center gap-2">
        <Button
          ...
          className="cursor-pointer gap-1 border-border h-9 text-muted-foreground hover:text-foreground font-medium rounded-lg"
        >
          ...
          {Array.from({ length: totalPages }).map((_, index) => {
            ...
            return (
              <Button
                ...
                className={`w-9 h-9 cursor-pointer font-medium rounded-lg ${
