...
import { usePublicCoursesQuery, usePublicExamsQuery, usePublicStreamsQuery } from "./_queries/courses.queries";
...
export default function CoursesPage() {
  const { data, isLoading } = usePublicCoursesQuery({ limit: 100 });
  ...
  const processedCourses = useMemo(() => {
    ...
    if (selectedExams.length > 0) {
      result = result.filter((c) => c.examName && selectedExams.includes(c.examName));
    }

    // Stream filter
    if (selectedStreams.length > 0) {
      result = result.filter((c) => c.streamName?.some((s) => selectedStreams.includes(s)));
    }
    ...
    return result;
  }, [coursesList, searchQuery, selectedExams, selectedStreams, sortBy]);
      ...
      <div className="min-h-screen bg-slate-50/20 dark:bg-slate-950/5 flex flex-col lg:flex-row pt-16">
        ...
        <main className="flex-1 p-6 md:p-8 lg:p-10 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-805/80 rounded-2xl p-5 space-y-4 animate-pulse flex flex-col h-full justify-between">
                <div>
      ...
      <section className="w-full relative overflow-hidden bg-gradient-to-b from-white via-violet-50/40 to-violet-100/50 dark:from-slate-950 dark:via-violet-955/10 dark:to-violet-900/10 py-16 md:py-20 select-none animate-in fade-in duration-300">
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 relative z-10 w-full flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 leading-relaxed font-semibold max-w-xl mx-auto">
              Get the only NCET preparation courses taught by actual IITians.
            </p>
          </div>

          {/* Centered Search Input & Popular Searches */}
          <div className="max-w-2xl w-full">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm rounded-2xl p-1.5 flex items-center w-full gap-2">
        ...
        <div className="absolute bottom-0 left-0 right-0 w-full overflow-hidden leading-none z-10 pointer-events-none translate-y-[1px]">
          <svg className="relative block w-full h-8 md:h-12 text-slate-50/20 dark:text-slate-950/5" viewBox="0 0 1200 120" preserveAspectRatio="none" fill="currentColor">
            ...
            <path d="M985.66,92.83C906.67,72,823.78,31,741.84,14.19c-82.26-17.34-168.06-16.33-250.45.39-57.84,11.73-114,31.07-172,41.86C245,69.66,165.63,40.65,82.1,22.81,53.05,16.63,24.9,8.75,0,0V120H1200V95.8C1132.19,118.9,1055.71,111.31,985.66,92.83Z" className="text-slate-50/50 dark:text-slate-950/20" />
          </svg>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 flex flex-col lg:flex-row gap-8 items-start min-h-screen">
        ...
        <div className="w-full lg:hidden flex gap-3 pb-2">
          <Button
            ...
            className="flex-1 h-11 bg-white hover:bg-slate-50 dark:bg-slate-900 dark:hover:bg-slate-850 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shadow-2xs cursor-pointer"
          >
        ...
        <main className="flex-1 w-full space-y-6">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center justify-between gap-4 p-4 bg-slate-50/50 dark:bg-slate-900/20 border border-slate-150/40 dark:border-slate-800/80 rounded-2xl">
              ...
              <button 
                ...
                className="inline-flex items-center gap-1.5 text-xs font-bold text-violet-600 dark:text-violet-400 hover:text-violet-750 transition-colors cursor-pointer"
              >
          ...
          <div className="flex items-center justify-between select-none">
            <span className="text-xs md:text-sm font-extrabold text-slate-800 dark:text-slate-200">
              {processedCourses.length} {processedCourses.length === 1 ? "Course" : "Courses"} Found
            </span>
          ...
          {processedCourses.length === 0 ? (
            <div className="w-full py-16 flex flex-col items-center justify-center text-center bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6">
              <div className="p-4 bg-slate-50 dark:bg-slate-850 rounded-full mb-4 border border-slate-150 dark:border-slate-800">
                <AlertCircle className="h-10 w-10 text-slate-400" />
              </div>
              <h3 className="text-lg font-extrabold text-slate-955 dark:text-slate-50 mb-1">No Courses Found</h3>
              <p className="text-xs text-slate-400 max-w-sm leading-relaxed mb-6">
                No active or upcoming courses match your criteria. Try adjusting your query or resetting filters.
              </p>
            ...
            <div className={activeView === "grid" ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "flex flex-col gap-6"}>
              {processedCourses.map((course) => (
                <CourseCard key={course.id} course={course} layout={activeView} />
              ))}
            </div>
          )}
        </main>
