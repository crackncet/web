...
export function EnrollmentDialog({ isOpen, onClose, course }: EnrollmentDialogProps) {
  ...
  const totalBatchPrice = Math.max(0, coursePrice + testSeriesPrice - couponDiscount);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await apiClient.post<any>("/payments/coupons/validate", {
        ...
        amount: coursePrice + testSeriesPrice,
      });
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-850 rounded-2xl p-5 space-y-4 shadow-xs">
                ...
                <div className="space-y-2">
                  ...
                  {!appliedCoupon ? (
                    ...
                    <div className="flex items-center justify-between px-3 h-9 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-250 dark:border-emerald-900/30 rounded-xl text-xs font-bold text-emerald-700 dark:text-emerald-400">
                      ...
                      <button
                        ...
                        className="text-[10px] font-black uppercase text-rose-600 dark:text-rose-400 hover:underline cursor-pointer"
                      >
                        Remove
                      </button>
                    </div>
                ...
                <div className="space-y-2 text-xs border-b border-slate-100 dark:border-slate-800 pb-3">
                  <div className="flex justify-between text-slate-550 dark:text-slate-400">
                    ...
                    <span className="font-bold text-slate-800 dark:text-slate-202">₹{coursePrice.toLocaleString("en-IN")}</span>
                  </div>
                  {course.testSeriesId && (
  ...
  if (isMobile) {
    return (
      <Sheet open={isOpen} onOpenChange={handleOpenChange} modal={!isModalBehaviorDisabled}>
        <SheetContent 
          side="bottom" 
          className="h-[75vh] max-h-[75vh] w-full overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden rounded-t-[2rem] bg-slate-50 dark:bg-slate-955 p-6 border-t border-slate-200 dark:border-slate-850 select-none"
