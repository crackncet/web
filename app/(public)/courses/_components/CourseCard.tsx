...
export function CourseCard({ course, layout = "grid" }: CourseCardProps) {
  ...
  if (layout === "list") {
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/80 rounded-3xl p-5 shadow-xs hover:shadow-md transition-all duration-300 group flex flex-col md:flex-row gap-6 select-none">
        {/* Banner container */}
        ...
        <div className="flex-1 flex flex-col justify-between py-1">
          <div className="space-y-2.5">
            <h3 className="text-base font-black text-slate-900 dark:text-slate-50 group-hover:text-violet-600 transition-colors">
              {course.title}
            </h3>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6 pt-4 border-t border-slate-100 dark:border-slate-800/80">
            ...
            <div className="w-full sm:w-auto">
              {course.isEnrollmentOpen ? (
                <Link href={`/courses/${course.id}`} className="block w-full sm:inline-block">
                  <button 
                    className="w-full sm:w-auto h-9 px-5 text-xs font-bold uppercase tracking-wider bg-violet-600 hover:bg-violet-750 text-white flex items-center justify-center gap-1.5 rounded-xl transition-all shadow-sm hover:shadow cursor-pointer"
                  >
