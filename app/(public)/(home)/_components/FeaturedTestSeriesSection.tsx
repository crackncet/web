...
import { FeaturedTestSeries } from "../_api/test-series.api";
import Link from "next/link";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import Image from "next/image";

const DUMMY_TEST_SERIES: FeaturedTestSeries[] = [
  {
    id: "dummy-ts-1",
    examName: "NCET 2026",
    name: "Complete NCET CBT Mock Test Series",
    description: "Get real exam-like practice with 10+ full length tests and subject-wise prep material designed by top teachers.",
    banner: null,
    price: "1499.00",
  ...
  },
  {
    id: "dummy-ts-2",
    examName: "NCET 2026",
    name: "General Test & Teaching Aptitude Mini Test Pack",
    description: "Focus on high-scoring sections with bite-sized chapter tests, key theory summaries and direct solution sheets.",
    banner: null,
    price: "499.00",
...
interface FeaturedTestSeriesSectionProps {
  testSeries: FeaturedTestSeries[];
  isLoading: boolean;
}

export default function FeaturedTestSeriesSection({
  testSeries,
  isLoading,
}: FeaturedTestSeriesSectionProps) {
  const [visibleCount, setVisibleCount] = useState(3);
  const [currentIndex, setCurrentIndex] = useState(0);

  const displayTestSeries = testSeries.length > 0 ? testSeries : DUMMY_TEST_SERIES;
  ...
  const maxIndex = Math.max(0, displayTestSeries.length - visibleCount);
  const showNavigation = displayTestSeries.length > visibleCount;
  const shouldCenter = displayTestSeries.length < visibleCount;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-12 md:mb-16">
          ...
          <h2 className="text-3xl md:text-5xl font-extrabold text-foreground tracking-tight">
            Featured <span className="text-primary">Test Series</span>
          </h2>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto px-4 md:px-12">
          ...
          {showNavigation && (
            <>
              ...
              <button
                ...
                aria-label="Previous Test Series"
              >
          ...
          <div className="overflow-hidden w-full">
            ...
            >
              {displayTestSeries.map((ts) => {
                const formattedPrice = parseFloat(ts.price).toLocaleString("en-IN");
                const dateStr = ts.startDate
                  ? `Starts: ${new Date(ts.startDate).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                      year: "numeric",
                    })}`
                      ...
                      <CardContent className="relative bg-white dark:bg-slate-900 pt-5 px-5 pb-5 flex-1 flex flex-col justify-between -mt-4 z-20">
                        ...
                        <div className="space-y-3.5 mt-1">
                          ...
                          <div className="flex flex-wrap items-center justify-center gap-1.5 min-h-[20px]">
                            {ts.streams && ts.streams.length > 0 ? (
                              ts.streams.map((stream: string, idx: number) => (
                                <span
                                  key={idx}
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/60 space-y-3.5">
                          ...
                          <div className="pt-0.5">
                            <Link href={`/test-series/${ts.id}`} className="w-full block">
                              <Button 
